# -*- coding: utf-8 -*-
"""TSI_project_join.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1h7p7jc8zjRO0T7CYYRor0nL2fT2tpahE
"""
...
# BLOCCO MAIN
stringa1 = 'SELECT nome, cognome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB'
stringa11 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB'
stringa111 = 'SELECT * FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB'

stringa2 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.attribute > 0'
stringa22 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.attribute > 0 AND tableB.attr = 0'
stringa222 = 'SELECT nome, cognome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.attribute > 0 AND tableB.attr = 0'
stringa2222 = 'SELECT * FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.attribute > 0 AND tableB.attr = 0'

stringa3 = 'SELECT nome, cognome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.attribute > 0 AND tableB.attr = 0 limit 2'

stringa4 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.user_id like "%bc%"' 
stringa44 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.user_id like "bc%" and tableB.attr = 0' 
stringa444 = 'SELECT nome FROM tableA LEFT JOIN tableB ON tableA.fieldA = tableB.fieldB WHERE tableA.user_id like "%bc"' 
...
db.tableA.aggregate([
    { $lookup:
        {
           from: "tableB",                   
           localField: "fieldA",        FIELD TABLE A
           foreignField: "fieldB",      FIELD TABLE B
           as: "address"                OUTPUT FIELD 
        }
    },
...
######## Controllare cosa ci va su AS all'interno del LOOKUP e controllare se la query funziona in MongoDB anche se i nomi non hanno gli apici
...
# (es) parsed -> ['status', '=', '"D"', 'OR', 'name', '<=', '"Carlo"', 'AND', 'name', '!=', '"Saretta"']
  for token in tokens:
    ...
    if isinstance(token, Identifier) and not select_found and not order_by_found and not table_name_found:
      table = token.value
    ...
    if token.value == "LEFT JOIN":
      left_join_found = True
    if isinstance(token, Identifier) and left_join_found:
      second_table = token.value
    if isinstance(token, Comparison) and left_join_found:
      left_join_found = False
      output_parenthesis_left_join = convert_left_join(token, second_table)
    ...
    if isinstance(token, Where):
      ...
      if isinstance(output[0],LogicOperator):
        ...
        if limit_found:
          limit_found = False
          final_query = "db."+ table +".aggregate(["+ output_parenthesis_left_join + comma + " { $match: " + output[-1].created_string + "}" + output_parenthesis_select_attributes + "])" + output_parenthesis_limit
        else:
          final_query = "db."+ table +".aggregate(["+ output_parenthesis_left_join + comma + " { $match: " + output[-1].created_string + "}" + output_parenthesis_select_attributes + "])"
        ...
        final_query = "db." + table + ".aggregate([" + output_parenthesis_left_join + comma + " { $match: {" + output[0] + ": " + like_arg + "}}" + output_parenthesis_select_attributes + "])"   ######### RICONTROLLA QUESTO OUTPUT
        ...
        if limit_found:
          limit_found = False
          final_query = "db." + table + ".aggregate([" + output_parenthesis_left_join + comma + " { $match: " + str(output_parenthesis[0]) + "}" + output_parenthesis_select_attributes + "])" + output_parenthesis_limit
        else:
          final_query = "db." + table + ".aggregate(["  + output_parenthesis_left_join + comma + " { $match: "+ str(output_parenthesis[0]) + "}" + output_parenthesis_select_attributes + "])"
  ...
  if not where_found:
    if wildcard_found:
      final_query = "db."+str(table)+".aggregate([" + output_parenthesis_left_join + "])"
    elif output_parenthesis_select_attributes != "" :
      final_query = "db."+str(table)+".aggregate([" + output_parenthesis_left_join + output_parenthesis_select_attributes + "])"
...
def convert_single_select_attribute(token):
  if (token.value == "id"):
    output_parenthesis_select = ", { $project: {_id: 1} }"
  else:
    output_parenthesis_select = ", { $project: { " + token.value + ": 1, " + "_id: 0} }"
...
def convert_multiple_select_attributes(token):
  ...
  if not id_found:
    output_parenthesis_select = ", { $project: { " + output_string_noquotes +  ", _id: 0 } }"
  else: 
    output_parenthesis_select = ", { $project: { " + output_string_noquotes +  " } }"

  return output_parenthesis_select

def convert_left_join(token, second_table):
  """
  { $lookup:
        {
           from: tableB,                   
           localField: fieldA,        FIELD TABLE A
           foreignField: fieldB,              FIELD TABLE B
           as: address                      OUTPUT FIELD --> ??? controlla
        }
    }
    """
  outputField = "output_field" 
  output_parenthesis_left_join = "{ $lookup: { from: " + second_table + ", localField: " + str(token[0][2]) + ", foreignField: " + str(token[4][2]) + ", as: " +outputField + " }}"

  return output_parenthesis_left_join